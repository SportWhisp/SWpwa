...
// --- Funzioni di supporto (copiate da genAnalysis.js) ---
function initStats() {
...
export default function handler(req, res) {
  try {
    ...
    // ✅ Percorsi file specifici per stagione 2024-2025
    const inputPath = path.join(process.cwd(), `data/${league}/results/results-${league}-2024-2025.json`);
    const outputPath = path.join(process.cwd(), `data/${league}/Sto${league.charAt(0).toUpperCase() + league.slice(1)}.json`);
    ...
    return res.status(200).json({
      message: `✅ Sto${league.charAt(0).toUpperCase() + league.slice(1)}.json generato`,
    ...
    console.error(err);
    return res.status(500).json({ error: "Errore durante la generazione del file storico" });
  }
}