...
export default function handler(req, res) {
  ...
  const stagione = req.query.season || "2025-2026";
  ...
  try {
    ...
    for (const match of partite) {
      ...
      // 🔍 Se è una partita conclusa, la aggiungiamo con statistiche vere
      if (match.status?.toLowerCase() === "finished") {
        ...
        // 🆘 Se la partita non è finita, assicuriamoci che le squadre siano comunque presenti con array vuoti
        if (!teams[home]) {
          teams[home] = {
            team: home,
            totale: [],
    ...
    res.status(500).json({ error: "Errore nella generazione del file di analisi" });
  }
}
