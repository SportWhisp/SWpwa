...
export default function RisultatiSuperLigAdmin() {
  ...
  useEffect(() => {
    fetch(`/api/readResult?league=${LEAGUE}&file=${FILE_NAME}`)
      ...
      .then((data) => {
        ...
        // ✅ Parsing dei risultati API-Football per Süper Lig
        if (data?.response) {
      ...
      .catch((err) => console.error("❌ Errore nel caricamento risultati Süper Lig:", err));
  }, []);

  return (
    <Layout>
      <h1>Risultati Süper Lig {SEASON} (ADMIN)</h1>
